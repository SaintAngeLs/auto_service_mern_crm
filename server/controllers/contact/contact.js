...
const addPropertyInterest = async (req, res) => {
    try {
        const { id } = req.params
        await Contact.updateOne({ _id: id }, { $set: { interestVehicle: req.body } });
...
const addVehicleInterest = async (req, res) => {
    try {
        const { id } = req.params;
        await Contact.updateOne({ _id: id }, { $set: { interestVehicle: req.body } });
        res.send('Vehicle interest uploaded successfully.');
    } catch (err) {
        console.error('Failed to add vehicle interest:', err);
        res.status(400).json({ error: 'Failed to add vehicle interest' });
    }
}
...
const view = async (req, res) => {
    try {
        ...
        let interestVehicle = await Contact.findOne({ _id: req.params.id }).populate("interestVehicle")
        ...
        res.status(200).json({ interestVehicle, contact, EmailHistory, phoneCallHistory, meetingHistory, textMsg, task, Document });
    }
    catch (error) {
        console.error(error);
...
module.exports = { index, add, addPropertyInterest, addVehicleInterest, view, edit, deleteData, deleteMany }